...
export default function NearbyScreen() {
  ...
  return (
    <main className="screen">
      ...
      {state.kind === 'loading' && <div className="empty">{state.message}</div>}
      {state.kind === 'error' && <div className="banner">{state.message}</div>}
      ...
      {state.kind === 'error' && (
        <button type="button" className="btn" style={{ marginTop: 16 }} onClick={load}>
          다시 시도
        </button>
      )}
    </main>
